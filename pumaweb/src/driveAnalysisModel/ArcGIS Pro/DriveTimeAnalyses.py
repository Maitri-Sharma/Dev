...
ReolMap = r"C:\Workspace\AnalysisServices\DriveTime\KSPU.sde\puma.kspu_gdb.norway_reol"
...
ReolLayer = arcpy.MakeFeatureLayer_management(ReolMap, "ReolLayer", "", "", fields)
...
if AnalysisType == "DriveTime" or AnalysisType == "DriveDistance":
    ...
    if AnalysisType == "DriveTime":
        ServiceAreaLayer = arcpy.MakeServiceAreaLayer_na(Vegnett, "ServiceAreaLayer", TimeName, "TRAVEL_TO", BreakValue, "SIMPLE_POLYS", "NO_MERGE", "RINGS", "NO_LINES", "OVERLAP", "NO_SPLIT", "", "", "ALLOW_UTURNS", "OneWay", "NO_TRIM_POLYS", "100 Meters", "NO_LINES_SOURCE_FIELDS")
    else:
        ServiceAreaLayer = arcpy.MakeServiceAreaLayer_na(Vegnett, "ServiceAreaLayer", LengthName, "TRAVEL_TO", BreakValue, "SIMPLE_POLYS", "NO_MERGE", "RINGS", "NO_LINES", "OVERLAP", "NO_SPLIT", "", "", "ALLOW_UTURNS", "OneWay", "NO_TRIM_POLYS", "100 Meters", "NO_LINES_SOURCE_FIELDS")
    arcpy.AddMessage("Created service area layer. Adding locations.")
    arcpy.AddLocations_na(ServiceAreaLayer, "Facilities", StartingPoints, "Name Name #;CurbApproach # 0;Attr_Time # 0;Attr_Length # 0", "200 Meters", "", "ELVEG_VegnettLinje SHAPE", "MATCH_TO_CLOSEST", "APPEND", "NO_SNAP", "5 Meters", "INCLUDE", "ELVEG_VegnettLinje #")
    arcpy.AddMessage("Added locations. Solving.")

    arcpy.AddMessage("PROCESSING")
    arcpy.Solve_na(ServiceAreaLayer, "SKIP", "TERMINATE")
    x = 0
    ...
    arcpy.SelectLayerByLocation_management(ReolLayer, "INTERSECT", ServiceArea, "", "NEW_SELECTION")
    arcpy.AddMessage("Selected ruter intersecting service area polygons. Creating result table.")
    Result = arcpy.CreateUniqueName("ResultTable", "memory")
    arcpy.CopyRows_management(ReolLayer, Result, "")
    ...
    if AnalysisType == "ITRTime":
        ServiceAreaLayer = arcpy.MakeServiceAreaLayer_na(Vegnett, "ServiceAreaLayer", TimeName, "TRAVEL_TO", BreakValue, "NO_POLYS", "NO_MERGE", "RINGS", "TRUE_LINES_WITH_MEASURES", "OVERLAP", "NO_SPLIT", "", LengthName + ";" + TimeName, "ALLOW_UTURNS", "OneWay", "TRIM_POLYS", "100 Meters", "NO_LINES_SOURCE_FIELDS")
    else:
        ServiceAreaLayer = arcpy.MakeServiceAreaLayer_na(Vegnett, "ServiceAreaLayer", LengthName, "TRAVEL_TO", BreakValue, "NO_POLYS", "NO_MERGE", "RINGS", "TRUE_LINES_WITH_MEASURES", "OVERLAP", "NO_SPLIT", "", LengthName + ";" + TimeName, "ALLOW_UTURNS", "OneWay", "TRIM_POLYS", "100 Meters", "NO_LINES_SOURCE_FIELDS")
    arcpy.AddMessage("Created service area layer. Adding locations.")
    arcpy.AddLocations_na(ServiceAreaLayer, "Facilities", StartingPoints, "CurbApproach # 0;Attr_Length # 0;Attr_Time # 0;Name Name #", "200 Meters", "", "ELVEG_VegnettLinje SHAPE", "MATCH_TO_CLOSEST", "APPEND", "NO_SNAP", "5 Meters", "INCLUDE", "ELVEG_VegnettLinje #")
    arcpy.AddMessage("Added locations. Solving.")
    ...
    ReolerWithTimeAndDistance = arcpy.CreateUniqueName("ReolerWithTimeAndDistance", "memory")
    ReolerWithTimeAndDistance = arcpy.Identity_analysis(ServiceAreaLines, ReolLayer, ReolerWithTimeAndDistance, "ALL", "", "NO_RELATIONSHIPS")
    #fields = ""
    ...
    Result = arcpy.Statistics_analysis(ReolerWithTimeAndDistance, Result, "FromCumul_" + TimeName + " MIN;ToCumul_" + TimeName + " MIN;ToCumul_" + LengthName + " MIN;FromCumul_" + LengthName + " MIN", "REOL_ID")
    arcpy.SetParameter(2, Result)
else:
