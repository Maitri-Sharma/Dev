...
using Puma.Infrastructure.Interface.SQLHelper;
using System;
...
namespace Puma.Infrastructure.SQLDbHelper
{
    /// <summary>
        ...
        where TAdapter : IDbDataAdapter, IDisposable, new()
    {
        ...
        public DataSet GetlDataSet(string sql, CommandType commandType, params NpgsqlParameter[] parameters)
        {
            return GetDataSets(sql,parameters);
        }
        ...
        /// <summary>
        /// First pass number of cursor with name you want to return
        /// </summary>
        /// <param name="parameters"></param>
        /// <returns></returns>
        public DataSet GetDataSets(string functionName, params NpgsqlParameter[] parameters)
        {
            var dataSet = new DataSet();
            ...
            //Prepare query for ref cursor

            //First write query to fetch cursors from function


            string queryToFetchFromCursor = string.Empty;
            ...
            if (parameters != null)
            {
                ...
                {
                    foreach (NpgsqlParameter dbDataParameter in parameters)
                        if (dbDataParameter.NpgsqlDbType == NpgsqlDbType.Refcursor)
                        {
                            queryToFetchFromCursor += " " + "FETCH ALL FROM " + dbDataParameter.Value.ToString().ToLower() + ";";

                            if (!string.IsNullOrWhiteSpace(cursorName))
                            {
                                cursorName += "," + "'" + dbDataParameter.Value.ToString().ToLower() + "'";
                            }
                            else
                                cursorName += "'" + dbDataParameter.Value.ToString().ToLower() + "'";
                        }
                        else
                        {
                            if (!string.IsNullOrWhiteSpace(cursorName))
                            {
                                cursorName += "," + "'" + dbDataParameter.Value.ToString() + "'";
                            }
                            else
                                cursorName += "'" + dbDataParameter.Value.ToString() + "'";
                        }
            ...
            string query = "select * from " + functionName + "(" + cursorName + ");";
            query += queryToFetchFromCursor;
            ...
            {
                //Remove first table from dataset as it will always return cursor table
        ...
        /// <returns>The value returned from the function argument.</returns>
        /// 
        public ulong WriteToDatabase<T>(PostgreSQLCopyHelper<T> copyHelper, IEnumerable<T> entities)
        {
